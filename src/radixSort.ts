import radix_shader from "./shaders/radixSort.wgsl?raw";
...
// Just for uniforms in radixSort, Uint8Array decompisition is not the same as bytemuck
function createBufferInit(device: GPUDevice, descriptor: BufferInit): GPUBuffer {
...
function upload_to_buffer(encoder: GPUCommandEncoder, buffer: GPUBuffer, device: GPUDevice, values: ArrayBuffer) {
...
export async function guessWorkGroupSize(device: GPUDevice, queue: GPUQueue): Promise<number> {
...
export class GPUSorter {
    ...
    constructor(device: GPUDevice, subgroup_size: number, timer: Timer = null) {
        ...
        if (timer != null) {
            timer.subscribe("sortHistogramTime");
            timer.subscribe("sortPrefixTime");
            timer.subscribe("sortScatterTime");
        }
    }
    ...
    private record_calculate_histogram_indirect(bind_group: GPUBindGroup, dispatch_buffer: GPUBuffer, encoder: GPUCommandEncoder, timer: Timer): void {
        {
            let pass: GPUComputePassEncoder = encoder.beginComputePass({
                label: "zeroing the histogram",
                timestampWrites: timer.getSubscription("sortHistogramTime")
            } as GPUComputePassDescriptor);
    ...
    private record_prefix_histogram(bind_group: GPUBindGroup, encoder: GPUCommandEncoder, timer: Timer = null): void {
        let pass: GPUComputePassEncoder = encoder.beginComputePass({
            label: "prefix histogram",
            timestampWrites: (timer == null) ? undefined : timer.getSubscription("sortPrefixTime")
        } as GPUComputePassDescriptor);;
    ...
    private record_scatter_keys_indirect(bind_group: GPUBindGroup, dispatch_buffer: GPUBuffer, encoder: GPUCommandEncoder, timer: Timer): void {
        let pass: GPUComputePassEncoder = encoder.beginComputePass({
            label: "radix sort scatter keyvals",
            timestampWrites: timer.getSubscription("sortScatterTime")
        } as GPUComputePassDescriptor);
    ...
    public sort_indirect(encoder: GPUCommandEncoder, sort_buffers: SortBuffers, dispatch_buffer: GPUBuffer, timer: Timer = null): void {
        let bind_group: GPUBindGroup = sort_buffers.bind_group;

        this.record_calculate_histogram_indirect(bind_group, dispatch_buffer, encoder, timer);
        this.record_prefix_histogram(bind_group, encoder, timer);
        this.record_scatter_keys_indirect(bind_group, dispatch_buffer, encoder, timer);
    }
